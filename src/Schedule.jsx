...
const Schedule = ({ data: { rooms, sessions, startTime, endTime } }) => {
  const minuteDiff = dayjs(endTime).diff(dayjs(startTime), "m");
  ...
  const sessionsTile = sessions.map((session) => {
    ...
    const props = {
      ...
      gridRowStart: dayjs(session.start).diff(dayjs(startTime), "m") / 5 + 1,
      gridRowEnd: dayjs(session.end).diff(dayjs(startTime), "m") / 5 + 1,
    };
